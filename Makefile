...
submodules:
	@if [ ! -e "$(LIBFTX_DIR)/.git" ]; then																																\
		...
		git -C $(LIBFTX_DIR) submodule update --init $(SRC_DIR)/get_next_line;																							\
		git -C $(LIBFTX_DIR)/$(SRC_DIR)/get_next_line checkout $$(git config -f $(abspath $(LIBFTX_DIR))/.gitmodules submodule.src/get_next_line.branch || echo main);	\
	fi
